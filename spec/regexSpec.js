describe("your regexes", function(){
  ...
  it("should match ip addresses", function(){
    ...
    var regex = /(([0-1][0-9][0-9])\.|(2[0-4][0-9])\.|(25[0-5])\.|([0-9][0-9])\.|([0-9])\.)(([0-1][0-9][0-9])\.|(2[0-4][0-9])\.|(25[0-5])\.|([0-9][0-9])\.|([0-9])\.)(([0-1][0-9][0-9])\.|(2[0-4][0-9])\.|(25[0-5])\.|([0-9][0-9])\.|([0-9])\.)(([0-1][0-9][0-9])|(2[0-4][0-9])|(25[0-5])|([0-9][0-9])|([0-9]))/;
  ...
  it("should detect nearby words", function(){
    ...
    var containsNearbyWords = function(haystack, needle){
      var haystackAry = haystack.split(/[\,\.\s]/);
      for (var i = 0; i < haystackAry.length; i++) {
        if (haystackAry[i] === needle) {
          for (var j = i+1; j <= i+3; j++) {
            if (haystackAry[i] === haystackAry[j]) {
              return true;
            }
          }
        }
